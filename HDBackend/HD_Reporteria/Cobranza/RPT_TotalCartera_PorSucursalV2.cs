...
{
    /// <summary>
    /// Version ejecutiva del resumen de cartera por sucursal.
    /// Archivo aparte: no sustituye a RPT_TotalCartera_PorSucursal.
    /// </summary>
    public class RPT_TotalCartera_PorSucursalV2
    {
        // ==================================================================
        //  PARAMETROS RAPIDOS  (cambiar aqui, sin tocar el resto del codigo)
        // ==================================================================

        /// <summary>Segunda hoja: true = solo sucursales con saldo vencido. false = todas.</summary>
        public static readonly bool SOLO_SUCURSALES_CON_VENCIDO = true;
        ...
        /// <summary>Muestra la columna con el numero de sucursal.</summary>
        public static readonly bool MOSTRAR_ID_SUCURSAL = true;
        ...
        /// <summary>Aire arriba del renglon de totales.</summary>
        private const float ESPACIO_GRUPO = 9f;

        /// <summary>Titulos del encabezado.</summary>
        public const string TITULO_HOJA1 = "CARTERA POR SUCURSAL";
        public const string TITULO_HOJA2 = "CARTERA VENCIDA POR SUCURSAL";
        ...
        public static RPT_Result Generar(IEnumerable<mdlCob_TotalCarteraPorSucursal> resumen, bool? soloConVencido = null)
        {
            try
            {
                ...
                bool filtrarVencido = soloConVencido ?? SOLO_SUCURSALES_CON_VENCIDO;
                string fontFamily = "Calibri";

                // El origen agrega un renglon "TOTALES"; aqui se descarta y los
                // totales se recalculan.
                var lista = (resumen ?? Enumerable.Empty<mdlCob_TotalCarteraPorSucursal>())
                    .Where(x => !EsRenglonTotal(x.sucursal))
                    .ToList();
                ...
                {
                    // ============================================================
                    //  HOJA(S) 1 : RESUMEN POR SUCURSAL
                    // ============================================================
                    document.Page(page =>
                    {
                        page.Size(PageSizes.A4.Landscape());

                        page.Header().Element(c => Encabezado(c, TITULO_HOJA1, fontFamily));
                        ...
                        page.Content().PaddingTop(6).PaddingLeft(30).PaddingRight(30).Column(col1 =>
                        {
                            col1.Item().Element(c => BandaContexto(c, fontFamily, new[]
                            {
                                ...
                                ("SUCURSALES", lista.Count.ToString("N0"), "", false),
                            ...
                            col1.Item().PaddingTop(8).Table(tabla =>
                            {
                                ...
                                {
                                    if (MOSTRAR_ID_SUCURSAL) c.ConstantColumn(26); // #
                                    c.RelativeColumn(1);    // sucursal
                                ...
                                {
                                    // --- primer nivel ---
                                    CeldaGrupo(header.Cell().ColumnSpan(MOSTRAR_ID_SUCURSAL ? 2u : 1u), "SUCURSAL", fontFamily);
                                    ...
                                    // --- segundo nivel ---
                                    if (MOSTRAR_ID_SUCURSAL) CeldaTitulo(header.Cell(), "#", fontFamily, false);
                                    CeldaTitulo(header.Cell(), "NOMBRE", fontFamily, false);
                                ...
                                int i = 0;
                                foreach (var mdl in lista)
                                {
                                    string fondo = (i++ % 2 == 1) ? ZEBRA : BLANCO;
                                    double cartera = Cartera(mdl);

                                    if (MOSTRAR_ID_SUCURSAL)
                                        Celda(tabla.Cell(), fondo, mdl.idsucursal.ToString(), fontFamily, false, false, SUAVE, 7.5f);
                                ...
                                FilaResumen(tabla, "TOTAL GENERAL  ·  " + lista.Count.ToString("N0") + (lista.Count == 1 ? " sucursal" : " sucursales"),
                                    fontFamily, gCartera, gSaldoFavor, gNeto, gJuridico, gActivo, gPorVencer, gVencido);
                            });
                        });

                        page.Footer().Element(c => PieDePagina(c, fontFamily, TITULO_HOJA1, true));
                    });
                    ...
                    document.Page(page =>
                    {
                        ...
                        page.Header().Element(c => Encabezado(c, TITULO_HOJA2, fontFamily));
                        ...
                        page.Content().PaddingTop(6).PaddingLeft(30).PaddingRight(30).Column(col1 =>
                        {
                            ...
                            {
                                ("CORTE AL", fechaCorte, "", false),
                                (filtrarVencido ? "SUCURSALES CON MORA" : "SUCURSALES", vencidas.Count.ToString("N0"), "", false),
                            ...
                            col1.Item().PaddingTop(8).Table(tabla =>
                            {
                                ...
                                int j = 0;
                                foreach (var mdl in vencidas)
                                {
                                    string fondo = (j++ % 2 == 1) ? ZEBRA : BLANCO;
                                ...
                                Celda(tabla.Cell().ColumnSpan(MOSTRAR_ID_SUCURSAL ? 2u : 1u), VERDE_OSC, "TOTAL VENCIDO", fontFamily, false, true, BLANCO, 8f, ESPACIO_GRUPO);
                        ...
                        page.Footer().Element(c => PieDePagina(c, fontFamily, TITULO_HOJA2, false));
                    });
                }).GeneratePdf();
        ...
        /// <summary>Renglon de total general.</summary>
        ...
        {
            string fondo = VERDE_OSC;
            string tinta = BLANCO;
            string tintaPct = "#cfe0c2";
            float size = 8f;
            float aire = ESPACIO_GRUPO;

            Celda(tabla.Cell().ColumnSpan(MOSTRAR_ID_SUCURSAL ? 2u : 1u), fondo, etiqueta, fontFamily, false, true, tinta, size, aire);
            Celda(tabla.Cell(), fondo, cartera.ToString("N2"), fontFamily, true, true, tinta, size, aire);
            Celda(tabla.Cell(), fondo, saldoFavor.ToString("N2"), fontFamily, true, true, "#ffd9d4", size, aire);
        ...
        private static double Cartera(mdlCob_TotalCarteraPorSucursal x) => x.totalcartera;
        private static double Neto(mdlCob_TotalCarteraPorSucursal x) => x.total;

        /// <summary>Detecta el renglon de totales que agrega el origen de datos.</summary>
        private static bool EsRenglonTotal(string? sucursal)
        {
            string s = (sucursal ?? "").Trim();
            return string.Equals(s, "TOTAL", StringComparison.OrdinalIgnoreCase)
                || string.Equals(s, "TOTALES", StringComparison.OrdinalIgnoreCase);
        }
