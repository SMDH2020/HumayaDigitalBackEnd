...
{
    public class XLS_EstadoResultadosTodos
    {
        public static Task<DocResult> EstadoResultados(Fmdl_EstadoResultados_PDF lista, string rutaArchivos)
        {
            try
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    workbook.SaveAs(rutaArchivos);

                }
                if (System.IO.File.Exists(rutaArchivos))
                {
                    byte[] docbytes = System.IO.File.ReadAllBytes(rutaArchivos);
