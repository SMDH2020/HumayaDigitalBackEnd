...
namespace HD.Endpoints.Controllers.Authenticate
{
    ...
    public class AutenticateMovilController : ControllerBase
    {
        private readonly IConfiguration Configuracion;

        public AutenticateMovilController(IConfiguration configuration)
        {
            Configuracion = configuration;
        }
        [HttpPost]
        public async Task<ActionResult> Post(mdlLogin_Movil mdl)
        {
            if (mdl is null)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await datos.AutenticarMovil(mdl);
                ...
                var token = await JwtManager.GenerarTocken(usuario, usuario, securitytkey, iussuer, audience, 10080,"APP");
