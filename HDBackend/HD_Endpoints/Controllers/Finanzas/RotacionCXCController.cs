...
namespace HD.Endpoints.Controllers.Finanzas
{
    public class RotacionCXCController : MyBase
    {
        ...
        public async Task<ActionResult> ReporteCXC(int ejercicio, int periodo, string adr, string sucursales)
        {
            string CadenaConexion = Configuracion["ConnectionStrings:Servicio"];
            AD_RotacionCXC_Reporte datos = new AD_RotacionCXC_Reporte(CadenaConexion);
            var usuario = Sesion.usuario();
            var result = await datos.reporte(ejercicio, periodo, adr, sucursales, usuario);
            return Ok(result);
        }
