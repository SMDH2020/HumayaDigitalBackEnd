...
namespace HD.Endpoints.Controllers.GestionCobranza
{
    public class ListadoGestionesRealizadasComentarioController : MyBase
    {
        ...
        [HttpGet]
        [Route("/api/[controller]/[action]")]
        public async Task<ActionResult> ImprimirExcel(string? fechainicio, string? fechafin, string adr, string sucursal, int responsable, string? titulo)
        {
            string CadenaConexion = Configuracion["ConnectionStrings:Servicio"];
            AD_Listado_Gestiones_Realizadas_Comentario datos = new AD_Listado_Gestiones_Realizadas_Comentario(CadenaConexion);
            var result = await datos.Get(fechainicio, fechafin, adr, sucursal, responsable);
        ...
        [HttpGet]
        [Route("/api/[controller]/[action]")]
        public async Task<ActionResult> ImprimirPDF(string? fechainicio, string? fechafin, string adr, string sucursal, int responsable, string? titulo)
        {
            string CadenaConexion = Configuracion["ConnectionStrings:Servicio"];
            AD_Listado_Gestiones_Realizadas_Comentario datos = new AD_Listado_Gestiones_Realizadas_Comentario(CadenaConexion);
            var result = await datos.Get(fechainicio, fechafin, adr, sucursal, responsable);
