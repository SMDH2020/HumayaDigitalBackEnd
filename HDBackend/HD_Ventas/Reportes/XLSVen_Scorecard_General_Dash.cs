...
{
    public static class XLSVen_Scorecard_General_Dash
    {
        ...
        public static Task<DocResult> GenerarExcel(IEnumerable<mdlCarga_Scorecard_porVendedor_Dash> scorecard, int ejercicio, int mes_actual, int ejercicio_inicio, int periodo_inicio)
        {
            try
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    int renglon = XLSEncabezado.Encabezado(ref sheet, $"SCORECARD GENERAL", 7);
                    ...
                    {
                        sheet.Cell(renglon, 1).Value = sco.linea;
                        sheet.Cell(renglon, 2).Value = sco.objetivo;
                        sheet.Cell(renglon, 3).Value = sco.unidades_vendidas;
                        //float porcentajemensual = sco.objetivo > 0 ? sco.unidades_vendidas / sco.objetivo : 0;
                        sheet.Cell(renglon, 4).FormulaA1 = $"=IF(B{renglon}>0, MIN(C{renglon}/B{renglon}, 1), 0)";
                        sheet.Cell(renglon, 5).Value = sco.objetivo_acumulado;
                        sheet.Cell(renglon, 6).Value = sco.unidades_vendidas_acumulado;
                        //float porcentajeacumulado = sco.objetivo_acumulado > 0 ? sco.unidades_vendidas_acumulado / sco.objetivo_acumulado : 0;
                        sheet.Cell(renglon, 7).FormulaA1 = $"=IF(F{renglon}>0, MIN(F{renglon}/E{renglon}, 1), 0)";
                        renglon++;
                    }
