...
{
    public class XLSCob_Listado_Clientes_Gestionar
    {
        ...
        public static Task<DocResult> GenerarExcel(IEnumerable<mdl_Listado_Clientes_Gestionar> detalle)
        {
            try
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    int renglon = XLSEncabezado.Encabezado(ref sheet, $"LISTADO DE CLIENTES A GESTIONAR", 8);
                    ...
                    sheet.Cell(renglon, 5).Value = "SALDO VENCIDO";
                    sheet.Cell(renglon, 6).Value = "SALDO POR VENCER";
                    sheet.Cell(renglon, 7).Value = "RESPONSABLE";
                    sheet.Cell(renglon, 8).Value = "GESTION";
                    ...
                    {
                        sheet.Cell(renglon, 1).Value = det.linea?.ToUpper();
                        sheet.Cell(renglon, 2).Value = det.estado;
                        sheet.Cell(renglon, 3).Value = det.sucursal;
                        sheet.Cell(renglon, 4).Value = det.RazonSocial?.ToUpper();
                        sheet.Cell(renglon, 5).Value = det.SaldoVencido;
                        sheet.Cell(renglon, 6).Value = det.SaldoporVencer;
                        sheet.Cell(renglon, 7).Value = det.ResponsableCobranza?.ToUpper();
                        sheet.Cell(renglon, 8).Value = det.gestion == "O" ? "PENDIENTE" :
