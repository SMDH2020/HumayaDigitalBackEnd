...
namespace HD_Cobranza.Capturas.ConvenioPago
{
    public class ADVencimientosSaldos
    {
        ...
        public async Task<IEnumerable<mdlVencidosOperacion>> ObtenerRevolventeob(int idcliente)
        {
            try
            {
                FactoryConection factory = new FactoryConection(CadenaConexion);
                ...
                IEnumerable<mdlVencidosOperacion> result = await factory.SQL.QueryAsync<mdlVencidosOperacion>("Cobranza.Saldo_Vencido_Por_Cliente_Revolvente", parametros, commandType: System.Data.CommandType.StoredProcedure);
                factory.SQL.Close();
                return result;
            }
            catch (System.Exception ex)
